...
const featureItems = [
  ...
  { label: '风险归因', href: '/event-analysis' },
];
      ...
      <nav className="glass-nav mx-auto flex max-w-[1680px] flex-col gap-3 px-4 py-3 sm:px-5 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <a
            ...
            className="rounded-full px-4 py-2 text-base font-medium text-muted-foreground transition duration-200 hover:-translate-y-0.5 hover:bg-white/70 hover:text-foreground"
          >
            {navItems[0].label}
          </a>

          <details className="group relative">
            <summary className="flex cursor-pointer list-none items-center gap-1 rounded-full px-4 py-2 text-base font-medium text-muted-foreground transition duration-200 hover:-translate-y-0.5 hover:bg-white/70 hover:text-foreground">
              <span>亮点功能</span>
            ...
            <div className="absolute right-0 top-[calc(100%+0.55rem)] z-20 min-w-[9rem] rounded-[22px] border border-white/75 bg-white/86 p-2 shadow-soft backdrop-blur-xl">
              {featureItems.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="block rounded-2xl px-3 py-2.5 text-base font-medium text-muted-foreground transition duration-200 hover:bg-background/90 hover:text-foreground"
                >
                  {item.label}
                </a>
              ))}
