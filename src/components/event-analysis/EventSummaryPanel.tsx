...
function EventSummaryPanel({
  ...
  return (
    <aside className={`surface-card relative h-full p-4 sm:p-5 ${isAttributionHelpOpen ? 'z-[120]' : 'z-0'}`}>
        ...
        <div className="min-w-0">
          ...
          <div className="relative mt-2">
            <select
              ...
              className="h-11 w-full appearance-none rounded-2xl border border-border/80 bg-white/85 px-4 pr-11 text-sm font-semibold text-foreground shadow-sm outline-none transition focus:border-accent/30"
            >
        ...
        <div className="min-w-0">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            <select
              ...
              className="h-11 rounded-2xl border border-border/80 bg-white/85 px-3 text-sm font-semibold text-foreground shadow-sm outline-none transition focus:border-accent/30"
            >
      ...
      <div className="mt-4 rounded-[24px] border border-border/75 bg-white/85 p-3">
        <div className="grid gap-2.5 sm:grid-cols-2">
          <div className="rounded-[20px] bg-background/90 px-3.5 py-2.5">
            <div className="text-sm text-muted-foreground">平均风扰指数</div>
            <div className="mt-1.5 text-4xl font-extrabold leading-none text-foreground">{formatWindDisturbanceIndex(airport.averageIndex)}</div>
          </div>
          <div className="rounded-[20px] bg-background/90 px-3.5 py-2.5">
            <div className="text-sm text-muted-foreground">降落风险数</div>
            <div className="mt-1.5 text-4xl font-extrabold leading-none text-foreground">{airport.eventCount}</div>
          </div>
        </div>
      </div>

      <div className="mt-4 rounded-[24px] border border-border/75 bg-white/85 p-3.5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <h3 className="text-xl font-bold text-foreground">风扰特征贡献排序</h3>
              ...
              {isAttributionHelpOpen && (
                <div className="absolute right-0 top-8 z-[999] w-[calc(100vw-2.5rem)] rounded-[20px] border border-border/70 bg-white p-5 text-xs leading-6 text-muted-foreground shadow-soft sm:left-full sm:right-auto sm:top-0 sm:ml-3 sm:w-[28rem]">
                  <div className="flex items-start justify-between gap-4">
                    <div className="text-sm font-bold text-foreground">归因计算流程</div>
                    <button
                      type="button"
                      aria-label="关闭归因说明"
                  ...
                  <p className="mt-2">
                    归因模型会逐项比较这些风扰特征对输出结果的贡献。页面里的百分比表示相对贡献大小，数值越高，说明这个特征在本月样本中越常参与解释操作响应的变化。
                  </p>
                  <p className="mt-2">
                    贡献靠前的风扰特征会继续进入后续解释模型，让结果保留可追溯的含义。
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-[160px_minmax(0,1fr)]">
          <div className="relative mx-auto h-[160px] w-[160px]">
            <svg viewBox="0 0 200 200" className="h-full w-full" role="img" aria-label={`${airport.name}${displayPeriod}归因饼图`}>
          ...
          <div className="space-y-1.5">
            {contributions.map((feature, index) => (
              <button
                ...
                onMouseLeave={() => setHoveredFeatureKey(null)}
                className="flex w-full items-center gap-2 rounded-[15px] bg-white/82 px-3 py-1.5 text-left transition hover:bg-white"
              >
                <span className="h-3 w-3 shrink-0 rounded-full" style={{ backgroundColor: feature.color }} />
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm font-semibold text-foreground sm:text-base">{index + 1}. {feature.label}</span>
                </span>
